...
class FirstScreen(tk.Tk):
    def __init__(self):
        ...
        background = Image.open("images/AppBG.png")
        ...
        head = tk.Label(self.first_frame, text="Sign Up", font="Kartika 17 bold", fg="#67d669")
        ...
        upload_button = tk.Button(self.first_frame, image=upload_image, compound="left", text="Upload Image",
                                  cursor="hand2", font="Kartika 12 bold", padx=2, command=self.add_photo)
        upload_button.place(x=345, y=220)

        self.username = tk.Label(self.first_frame, text="Username", font="Kartika 12 bold", bg="skyblue")
        self.username.place(x=80, y=150)

        self.username_entry = tk.Entry(self.first_frame,  font="Kartika 10 bold", width=10,
                                       highlightcolor="Blue", highlightthickness=1)
        self.username_entry.place(x=200, y=150)

        self.username_entry.focus_set()

        submit_button = tk.Button(self.first_frame, text="Connect", font="Kartika 12 bold", padx=30, cursor="hand2", bg="#a5faf4",
                                  command=self.process_data, relief="solid", bd=2)
    ...
    def process_data(self):
        if self.username_entry.get():
            ...
            try:
                client_socket.connect(("192.168.1.4", 50490))
...
class ChatScreen(tk.Canvas):
    def __init__(self, parent, first_frame, client_socket, clients_connected, user_id):
        super().__init__(parent, bg="#d9d5d4")
        ...
        #self.create_text(545, 120, text="Online", font="Kartika 12 bold", fill="#67d669")

        self.create_text(545, 120, text="Online", font="Kartika 12 bold", fill="green")

        tk.Label(self, text="   ", font="Kartika 15 bold", bg="#b5b3b3").place(x=4, y=29)
        #tk.Label(self, text="   ", font="Kartika 15 bold").place(x=4, y=29)

        tk.Label(self, text="Socket-based Chat", font="Kartika 15 bold", padx=20, fg="Green",
        ...
        send_button = tk.Button(self, text="Send", fg="white", font="Kartika 11 bold", bg="green", padx=10,
                                relief="solid", bd=2, command=self.sent_message_format)
        send_button.place(x=490, y=680)

        self.entry = tk.Text(self, font="Kartika 10 bold", width=38, height=2,
                             highlightcolor="blue", highlightthickness=1)
        ...
        t_label = tk.Label(m_frame, bg="#d9d5d4", text=datetime.now().strftime('%H:%M'), font="Kartika 9 bold")
        t_label.pack()

        m_label = tk.Label(m_frame, wraplength=250, text=f"Happy Chatting {self.parent.user}",
                           font="Kartika 10 bold", bg="orange")
        m_label.pack(fill="x")
        ...
        while True:
            try:
                ...
                if data_type == 'notification':
                    data_size = self.client_socket.recv(4) # 2048
    ...
    def received_message_format(self, data):
        ...
        t_label = tk.Label(m_frame, bg="#595656",fg="white", text=datetime.now().strftime('%H:%M'), font="Kartika 7 bold",
                           justify="left", anchor="w")
        t_label.grid(row=0, column=1, padx=2, sticky="w")

        m_label = tk.Label(m_frame, wraplength=250,fg="black", bg="#c5c7c9", text=message, font="Kartika 9 bold", justify="left",
                           anchor="w")
    ...
    def notification_format(self, data):
        ...
        t_label = tk.Label(m_frame, fg="black", bg="#d9d5d4", text=datetime.now().strftime('%H:%M'),
                           font="Kartika 9 bold")
        t_label.pack()

        m_label = tk.Label(m_frame, wraplength=250, text=message, font="Kartika 10 bold", justify="left", bg="sky blue")
        m_label.pack()
    ...
    def clients_online(self, new_added):
        if not new_added:
            ...
            for user_id in self.clients_connected:
                ...
                b = tk.Label(self, image=user, text=name, compound="left",fg="black", bg="#d9d5d4", font="Kartika 10 bold", padx=15)
                b.image = user
            ...
            b = tk.Label(self, image=user, text=name, compound="left", fg="black", bg="#d9d5d4",
                         font="Kartika 10 bold", padx=15)
            b.image = user
            self.clients_online_labels[user_id] = (b, self.y)
